    ...
    <M as Resource>::DynamicType: Default,
{
    ...
    pub async fn delete(name: &str, namespace: &str) -> Result<StatusCode> {
        let crd = Api::<M>::namespaced(Self::get_kube_client()?.clone(), namespace);
    ...
    pub async fn check_pod_running(name: &str, namespace: &str) -> Result<bool> {
    ...
    pub async fn check_pvc_exists(name: &str, namespace: &str) -> Result<bool> {
        let pvcs =
    ...
    pub async fn get_all_pods(namespace: &str) -> Result<Vec<Pod>> {
        let pods = Api::<Pod>::namespaced(Self::get_kube_client()?.clone(), namespace);
        let list = pods.list(&Default::default()).await?;
        Ok(list.items)
    }

    pub async fn get_pod_ip(name: &str, namespace: &str) -> Result<String> {
        let pods = Api::<Pod>::namespaced(Self::get_kube_client()?.clone(), namespace);
        let pod = pods.get(name).await?;
        Ok(pod
            .status
